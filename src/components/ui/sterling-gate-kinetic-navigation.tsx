...
export function Component() {
  ...
  // Init: set GSAP initial state (avoids CSS vs GSAP transform conflict)
  useEffect(() => {
    if (!containerRef.current) return;
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Safe fallbacks to avoid breaking the timeline
      ...
      if (isMenuOpen) {
        ...
        tl.set(navWrap, { display: "block" })
          // Slide the whole panel in from right
          .fromTo(menu, { xPercent: 120 }, { xPercent: 0, duration: 0.65, ease: "power3.out" }, "<")
          // Overlay fades in simultaneously
          .fromTo(overlay, { autoAlpha: 0 }, { autoAlpha: 1, duration: 0.4 }, "<")
          // Backdrop layers stagger-slide in for depth effect
          .fromTo(bgPanels, { xPercent: 101 }, { xPercent: 0, stagger: 0.1, duration: 0.5, ease: "power2.out" }, "<+=0.05")
          // Menu links reveal upward
          .fromTo(menuLinks, { yPercent: 140, rotate: 10, opacity: 0 }, { yPercent: 0, rotate: 0, opacity: 1, stagger: 0.07, duration: 0.5, ease: "back.out(1.2)" }, "<+=0.2");

        // Button text swap (safe)
        if (menuButtonTexts.length) {
          tl.fromTo(menuButtonTexts, { yPercent: 0 }, { yPercent: -100, stagger: 0.15, duration: 0.3 }, "<");
        }
        if (menuButtonIcon) {
        ...
        // Slide panel back out to the right
        tl.to(menu, { xPercent: 120, duration: 0.55, ease: "power3.in" })
          .to(overlay, { autoAlpha: 0, duration: 0.35 }, "<")
            ...
            <div className="menu-content-wrapper">
              <ul className="menu-list">
                ...
                <li className="menu-list-item" data-shape="4">
                  ...
                  >
                    <p className="nav-link-text" data-menu-fade>Project</p>
                    <div className="nav-link-hover-bg"></div>
                  </a>
                </li>
